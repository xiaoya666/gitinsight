 ...
 *              locks out a paying user. The real SPKI public key is embedded (PUBLIC_KEY_SPKI_B64URL),
 *              so verify() works today; it stays effectively dormant only because the activation UI is
 *              disabled and every 1.0.x user already runs as Pro Preview until 1.1.x flips that.
 *
 * @author: power
...
internal object LicenseVerifier {
    ...
    /**
     * base64url-encoded SPKI Ed25519 public key, embedded at release time. Generated by
     * gitinsight-server `npm run keygen`; the matching private key signs keys server-side and
     * never ships in the plugin. Rotate by regenerating the keypair and replacing this constant.
     */
    private const val PUBLIC_KEY_SPKI_B64URL = "MCowBQYDK2VwAyEAHp6rtS3tzfrrOiSz_qSohkjyIRpYTf7APxvMalx_ZcI"
